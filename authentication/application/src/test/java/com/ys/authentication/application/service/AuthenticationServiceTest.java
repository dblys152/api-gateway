...
class AuthenticationServiceTest {
    ...
    @BeforeEach
    void setUp() {
        PayloadInfo payloadInfo = PayloadInfo.of(ANY_EMAIL, ANY_USER_ID, ANY_NAME, UserRole.ROLE_USER.name());
    ...
    private User getMockUser() {
        ...
        given(user.getUserId()).willReturn(UserId.of(ANY_USER_ID));
    ...
    void 토큰을_갱신한다() {
        ...
        User user = getMockUser();
        given(loadUserPort.findByIdAndWithdrawnAtIsNull(UserId.of(ANY_USER_ID))).willReturn(Mono.just(user));
        ...
        StepVerifier.create(actual)
                .assertNext(tokenInfo -> {
                    assertAll(
                            ...
                            () -> then(loadUserPort).should().findByIdAndWithdrawnAtIsNull(UserId.of(ANY_USER_ID))
                    );
                })
                .verifyComplete();
    }
    ...
    void 토큰_갱신_시_인증정보의_유저가_존재하지_않으면_에러를_반환한다() {
        ...
        given(authenticationInfo.getUserId()).willReturn(ANY_USER_ID);
        given(loadAuthenticationInfoRedisPort.findByRefreshToken(REFRESH_TOKEN)).willReturn(Mono.just(authenticationInfo));
        given(loadUserPort.findByIdAndWithdrawnAtIsNull(UserId.of(ANY_USER_ID))).willThrow(NoSuchElementException.class);
    ...
    @Test
    void 토큰_페이로드를_조회한다() {
        PayloadInfo mockPayloadInfo = mock(PayloadInfo.class);
        given(identityProvider.getPayloadInfo()).willReturn(mockPayloadInfo);
        given(mockPayloadInfo.getUserId()).willReturn(ANY_USER_ID);
        ...
        StepVerifier.create(actual)
                .assertNext(payloadInfo -> {
                    ...
                    then(identityProvider).should().getPayloadInfo();
